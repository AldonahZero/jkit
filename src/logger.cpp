...
void init_logging(const std::string &log_path, logging::trivial::severity_level filter_level)
{
    ...
    boost::filesystem::path PathLogName(log_path);
    boost::system::error_code e;
    boost::filesystem::path parentPath = PathLogName.parent_path();
    boost::filesystem::path strstem = PathLogName.stem();
    boost::filesystem::path extensionName = PathLogName.extension();
    auto core = logging::core::get();
    boost::shared_ptr<sinks::text_file_backend> file_backend = boost::make_shared<sinks::text_file_backend>(
                keywords::file_name = parentPath.string() + "/" + strstem.string() + "%6N" + extensionName.string(),
